...
void setup(){
  ...
  unsigned long startWLANAttemptTime = millis();
  while (WiFi.status() != WL_CONNECTED && millis() - startWLANAttemptTime < 10000) {
    delay(500);
    Serial.print(".");
  }
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("\nWLAN-Verbindung hat nicht geklappt.");
  } else {
    Serial.printf("\nWLAN verbunden: SSID: %s, IP Address: %s\n", ssid, WiFi.localIP().toString().c_str());
