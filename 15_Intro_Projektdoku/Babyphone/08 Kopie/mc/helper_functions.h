...
int heulsession_id = 0;                                // HTTP POST request: id from the dataset in the database table will be stored here
    ...
    if (httpResponseCode > 0) {
        ...
        if (JSON.typeof(myObject) != "undefined") {
            if (myObject.hasOwnProperty("heulsession_id")) {
                ...
                Serial.print("Heulsession_id stored: ");
...
int getRandomTrackId() {
    ...
    int randomTrackId = selected_tracks_ids[randomTrackIndex];
    Serial.print("randomTrackId: ");
    Serial.println(randomTrackId);
