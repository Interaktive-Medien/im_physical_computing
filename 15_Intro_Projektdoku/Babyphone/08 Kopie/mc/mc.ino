...
#define AUDIOVOLUME_THRESHOLD 6
...
void loop(){
    ...
    if (is_heulsession == 1 && prev_is_heulsession == 0) {
        ...
        Serial.println("Heulsession started");
    }
    ...
    if (is_heulsession == 1 && !audio_played) {
        if (millis() - audiotrigger_startTime >= TIME_UNTIL_PLAY) { 
            ...
            String mext_track_title = getRandomTrackName();
...
void save_into_db(int is_heulsession){                       // save the start and end timestamp into database table
